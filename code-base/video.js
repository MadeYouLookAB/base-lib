class VideoLibrary {
  constructor(e = {}) {
    ((this.options = {
      ...
      scrollTriggerThreshold: e.scrollTriggerThreshold || 0.5,
      debug: e.debug || !1,
      ...e,
    }),
  ...
  init() {
    0 !== document.querySelectorAll("video[data-video]").length &&
  ...
  getComponentContainer(e) {
    return e.closest('[data-video="component"]') || e.parentElement || null;
  }
  removeDesktopOnlyVideos() {
    ...
    videos.forEach((video) => {
      const container = this.getComponentContainer(video),
        wrapper = container
          ? container.querySelector('[data-video-playback="wrapper"]')
          : null;
      isMobile
  ...
  setupLazyLoading() {
    ...
    ((this.videoObserver = new IntersectionObserver((entries, observer) => {
      ...
      videos.forEach((video) => {
        const scrollInPlay =
            "true" === video.getAttribute("data-video-scroll-in-play"),
          hover = "true" === video.getAttribute("data-video-hover");
      ...
      source && !source.src
        ? ((source.src = source.getAttribute("data-src")),
          ...
          video.addEventListener("canplaythrough", function handler() {
            (video.removeEventListener("canplaythrough", handler), resolve());
          }),
  ...
  showPictureElement(e) {
    const el = this.findPictureElement(e);
    el && (el.style.display = "block");
  }
  hidePictureElement(e) {
    const el = this.findPictureElement(e);
  ...
  findPictureElement(video) {
    ...
    let found = null,
      sibling = video.previousElementSibling;
    for (; sibling; ) {
      if ("PICTURE" === sibling.tagName || "IMG" === sibling.tagName) {
        found = sibling;
        break;
      }
    ...
    if (!found) {
      const container = this.getComponentContainer(video);
      container && (found = container.querySelector("picture, img"));
    }
    return (this.pictureElementCache.set(video, found), found);
  }
  setupVideoControls() {
  ...
  setupSoundControls() {
    document.querySelectorAll("video[data-video]").forEach((video) => {
      this.handleSoundButtons(video);
    });
  }
  setupHoverPlay() {
    document
      .querySelectorAll('video[data-video-hover="true"]')
      .forEach((video) => {
        const container = this.getComponentContainer(video),
          target = container || video;
        let hasPlayed = !1;
        if (!target) return;
        if ((this.showPictureElement(video), container)) {
            ...
            if (!this.prefersReducedMotion)
              try {
                (this.hidePictureElement(video),
                  ...
                  hasPlayed || ((video.currentTime = 0), (hasPlayed = !0)),
                  video.play());
              } catch (err) {
        ...
        (target.addEventListener("mouseenter", enterHandler),
          ...
          this.eventListeners.has(video) || this.eventListeners.set(video, []),
          this.eventListeners
            .get(video)
            .push(
              { element: target, type: "mouseenter", handler: enterHandler },
              { element: target, type: "mouseleave", handler: leaveHandler },
            ));
      });
  }
  setupAutoplay(video) {
    video.addEventListener("canplaythrough", () => {
      this.prefersReducedMotion ||
        (this.hidePictureElement(video), video.play().catch(console.error));
    });
  }
  setupScrollInPlay(video) {
    ...
    (observer.observe(video), this.scrollObservers.set(video, observer));
  }
      ...
      (entries) => {
        for (const entry of entries)
          entry.isIntersecting &&
            this.lazyLoadVideo(video)
              .then(() => {
                (video.pause(), this.showPictureElement(video));
              })
              .catch(console.error);
      },
      { threshold: this.options.scrollTriggerThreshold },
  ...
  handlePlaybackButtons(video) {
    const container = this.getComponentContainer(video);
    if (!container) return;
    const button = container.querySelector('[data-video-playback="button"]');
    if (!button) return;
    const playIcon = button.querySelector('[data-video-playback="play"]'),
      pauseIcon = button.querySelector('[data-video-playback="pause"]');
    ...
    updateState(!video.paused);
    const clickHandler = async (e) => {
        if ((e.stopPropagation(), video.paused))
          try {
            (await this.lazyLoadVideo(video),
              this.hidePictureElement(video),
              video.play(),
        ...
        else (video.pause(), updateState(!1));
      },
      playHandler = () => updateState(!0),
      pauseHandler = () => updateState(!1);
  ...
  handleSoundButtons(video) {
    const container = this.getComponentContainer(video);
    if (!container) return;
    const button = container.querySelector('[data-video-sound="button"]');
    if (!button) return;
    const muteIcon = button.querySelector('[data-video-sound="mute"]'),
      unmuteIcon = button.querySelector('[data-video-sound="unmute"]');
    if (!muteIcon || !unmuteIcon) return;
    const updateState = (isMuted) => {
      isMuted
    ...
    updateState(video.muted);
    const clickHandler = (e) => {
        e.stopPropagation();
        (video.muted &&
          document.querySelectorAll("video[data-video]").forEach((other) => {
            other !== video && (other.muted = !0);
          }),
          (video.muted = !video.muted),
          updateState(video.muted));
      },
      volumeChangeHandler = () => updateState(video.muted);
    (button.addEventListener("click", clickHandler),
      video.addEventListener("volumechange", volumeChangeHandler));
    const muteOnLeaveObserver = new IntersectionObserver(
      (entries) => {
        for (const entry of entries)
          entry.isIntersecting || video.muted || (video.muted = !0);
      },
      { threshold: 0 },
    );
    (muteOnLeaveObserver.observe(video),
      this.scrollObservers.set(
        `sound-${video.dataset.video || Math.random()}`,
        muteOnLeaveObserver,
      ),
      this.eventListeners.has(video) || this.eventListeners.set(video, []),
      this.eventListeners
  ...
  async playVideo(id) {
    ...
    if (video)
      try {
        if ((await this.lazyLoadVideo(video), this.prefersReducedMotion))
          return;
        (this.hidePictureElement(video), (video.currentTime = 0), video.play());
      } catch (err) {
        console.error(`Error playing video ${id}:`, err);
      }
    else console.warn(`Video with id "${id}" not found`);
  }
  pauseVideo(id) {
    const video = document.querySelector(`video[data-video="${id}"]`);
    video ? video.pause() : console.warn(`Video with id "${id}" not found`);
  }
  pauseAllVideos() {
    document.querySelectorAll("video[data-video]").forEach((video) => {
      video.pause();
  ...
  destroy() {
    (this.videoObserver && this.videoObserver.disconnect(),
      ...
      this.eventListeners.forEach((listeners) => {
        listeners.forEach(
          ({ element: element, type: type, handler: handler }) => {
...
function initializeVideoLibrary() {
  document.querySelectorAll("video[data-video]").length > 0
    ? (window.videoLibrary = new VideoLibrary())
    : window.VideoLibraryConfig?.debug &&
      console.log(
